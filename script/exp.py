...
'''
Total possible value in this sequnce is mod x mod 
The dataset below will generate all the pairs and we only take certian fraction of it.
Good and only works for seq_len = 2, and we got the desisred result with seq_len = 2 but now
we want to see how does this model react with small amount of data leak.
...
class FibonacciModDataset(Dataset):
    ...
    def generate_fib_sequence(self, mod):
        ...
        # right now I will keep this split like in the form of hit and trial.
        train_pairs = all_pairs[:int(0.09 * len(all_pairs))] 

        sequences = []

        for a, b in train_pairs:

            s = [a, b] # this is the starting pairs and we would like to generate more sequnece based on the seq_len 
...
file_name = f'batch_full_dimension_128_head_4_wd_0.09_with_mlp_wd_1_seq_21_intentional_leak_ds.pth'
